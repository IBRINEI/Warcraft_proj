...
class Entity:
    def __init__(self, canvas, param_dict, fraction, task, x, y):
        ...
        self.select_r = param_dict['select_r'] * 75 * canvas.scale
...
class Resource:
    ...
    def choose_image(self):
        if self.type == 'Gold':
            self.image = tk.PhotoImage(file='gold.png')
        elif self.type == 'Wood':
            self.image = tk.PhotoImage(file='wood.png')
        return self.image
...
class WarcraftApp(tk.Tk):
    def __init__(self):
        ...
        self.main_frame = MainFrame(self.master, self)
...
class BattleField(tk.Canvas):
    ...
    def building_place(self, building):
        if self.wood_counter >= entity_list[building]['wood_cost'] and\
        ...
        if not len(self.selecting_circles) == 0:
            for key in self.selecting_circles:
                if key.subtype == 'Factory':
                    if self.wood_counter >= entity_list[unit]['wood_cost'] and \
